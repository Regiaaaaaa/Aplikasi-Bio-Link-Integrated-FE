...
function SimpleCropper({ image, crop, zoom, onCropChange, onZoomChange, onCropComplete }) {
  ...
  useEffect(() => {
    if (imageSize.width > 0) {
      ...
      const pixelCrop = {
        x: Math.max(0, Math.round(((imageSize.width / 2 - cropSize / 2) - crop.x) * (imageSize.naturalWidth / imageSize.width))),
        y: Math.max(0, Math.round(((imageSize.height / 2 - cropSize / 2) - crop.y) * (imageSize.naturalHeight / imageSize.height))),
        width: Math.round(cropSize * (imageSize.naturalWidth / imageSize.width)),
        height: Math.round(cropSize * (imageSize.naturalHeight / imageSize.height))
      };
...
const handlePointerMove = (e) => {
  ...
  return (
    ...
    >
      <div className="relative" style={{ width: imageSize.width * zoom, height: imageSize.height * zoom }}>
        <img
          src={image}
          alt="Crop"
